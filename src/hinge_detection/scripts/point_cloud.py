...
class InputPointCloud:
    def __init__(self):
        self._pc_subscriber = rospy.Subscriber("/amazon_ws/hinge_detection/filtered_cloud", custom_pc_msg,
                                               self.init_pointcloud, queue_size=1)
    ...
    def init_pointcloud(self, input_msg):
        ...
        # self.filtered_cloud = np.array([(-2,2,2),(-2,-1,2),(-2,-1,3),(1,2,1),(0,0,0),(1,-1,1),(2,2,2)],
    ...
    def __compute_edge_length(self):
        ...
        # print(min_x_ind[-1][-1])
        self.__point_1 = np.array([self.sorted_cloud_x[min_x_ind[-1][-1]][0], self.sorted_cloud_x[min_x_ind[-1][-1]][1],
                                   self.sorted_cloud_x[min_x_ind[-1][-1]][2]])
        ...
        # self.__point_2 = np.array([self.sorted_cloud_y[min_x_ind[-1][-1]][0],self.sorted_cloud_y[min_x_ind[-1][
        # -1]][1],self.sorted_cloud_y[min_x_ind[-1][-1]][2]])
...
def main(args):
    rospy.init_node('bend_position', anonymous=True)
    ...
    plyfile_path = rospack.get_path('package_tracking') + "/data/optimized_data/" + args
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("usage: point_cloud.py arg1. TYPE AGAIN WITH RIGHT NUMBER OF ARGUMENTS. {arg1} is a file of type .ply")
    else:
        main(sys.argv[1])
